...
const JITSUGI_CATEGORY = '実技';

const EXACT_MAP = {
  'ライフプランニング・社会保険': 'ライフプランニング・社会保険',
  'ライフ・社会保険': 'ライフプランニング・社会保険',
  '社会保険': 'ライフプランニング・社会保険',
  'ライフプランニング': 'ライフプランニング・社会保険',
  '年金': 'ライフプランニング・社会保険',
  '職業倫理・関連法規': 'ライフプランニング・社会保険',
...
function fallbackMap(cat) {
  ...
  return 'ライフプランニング・社会保険';
}

function mapCategoryToDomain(cat) {
  if (!cat) return null;
  if (cat.includes('模擬試験')) return MOCK_EXAM_CATEGORY;
  if (cat.includes('実技')) return JITSUGI_CATEGORY;
  return EXACT_MAP[cat] || fallbackMap(cat);
}

module.exports = { DOMAINS, MOCK_EXAM_CATEGORY, JITSUGI_CATEGORY, mapCategoryToDomain };
